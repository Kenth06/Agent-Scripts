// ABOUTME: Chrome DevTools helper CLI for local browsing, eval, screenshots, and console capture.
// ABOUTME: Provides start/nav/eval/screenshot/pick/console/search/content/cookies/inspect/kill commands.
  ...
  .action(async (code: string[], options) => {
    ...
    try {
      const result = await page.evaluate((body) => {
        const AsyncFunction = Object.getPrototypeOf(async () => {}).constructor as AsyncFunctionCtor;
        return new AsyncFunction(`return (${body})`)();
      }, snippet);
...
async function ensureReadability(page: any) {
  ...
  const scripts = [
    ...
    'https://unpkg.com/turndown@2/dist/turndown.js',
